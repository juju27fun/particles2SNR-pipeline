...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Compute per-class Pearson matrices for transformed z8 parameters and "
            "validate their use as Cholesky dependence inputs."
        )
    )
    ...
    parser.add_argument(
        "--inclusive-difference-threshold",
        type=float,
        default=INCLUSIVE_DIFFERENCE_THRESHOLD,
    )
    ...
    args.inclusive_difference_threshold = validate_inclusive_difference_threshold(args.inclusive_difference_threshold)
    evidence = validate_method_evidence(workspace, evidence_id=args.method_evidence_id, evidence_run_id=args.method_evidence_run_id, contract_path=args.evidence_contract, dataset_id=args.dataset, method="cholesky_correlations")
    ...
    analysis = analyze_cholesky_correlations(
        rows,
        inclusive_difference_threshold=args.inclusive_difference_threshold,
    )
    outputs = write_cholesky_outputs(analysis, args.output_dir)

    repository = workspace.root / "particles2SNR-pipeline"
    module_path = repository / "particles2snr/z8_cholesky_analysis.py"
    script_path = repository / "scripts/analysis/analyze_z8_cholesky_correlations.py"
    git_states = {
    ...
    provenance = {
        ...
        "parameters": {
            ...
            "transformed_vector": list(PARAMETER_ORDER),
            "primary_population": "physical class rows from train and validation",
            "inclusive_population": (
                "physical and unclear rows mapped through physical_source_class"
            ),
            "correlation": "Pearson",
            "inclusive_difference_threshold": args.inclusive_difference_threshold,
            "regularization": (
                "minimal identity shrinkage preserving unit diagonal, only when "
                "the minimum eigenvalue is below 1e-10"
            ),
        },
        "metric_definitions": {
            "matrix_difference": "inclusive Pearson r minus physical Pearson r",
            "recommendation": (
                "inclusive when max absolute change over six coefficients is at most "
                "the configured threshold; otherwise physical dependencies with unclear "
                "extending only the SNR marginal"
            ),
            "condition_number": "largest singular value divided by smallest singular value",
            "cholesky_factor": (
                "lower triangular factor of the standardized regularized correlation matrix"
            ),
        },
        "code": {
            ...
            "particles2snr/z8_cholesky_analysis.py": sha256_file(module_path),
            "scripts/analysis/analyze_z8_cholesky_correlations.py": sha256_file(
                script_path
            ),
        },
    ...
    metric_names = [
        ...
        "coefficient_differences.csv",
        "matrix_diagnostics.csv",
        "cholesky_factors.csv",
        "recommendations.csv",
    ]
    metrics_manifest = {
        "schema_version": 1,
